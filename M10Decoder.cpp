...
M10Decoder::M10Decoder() {
    ...
    // Forward frame signal
    connect( m10Gtop, &M10GtopParser::sigFrame,
             this, &M10Decoder::sigFrame ) ;

    connect( m10Trimble, &M10TrimbleParser::sigFrame,
             this, &M10Decoder::sigFrame ) ;
...
int M10Decoder::startDecode(std::string fname) {
    ...
    audioFile = new AudioStream(fname.c_str(), baudRate);
    ...
    emit m10Parser->sigFrame( "<span style=\"color:red;\">Decoder has ended.</span>" ) ;
...
void M10Decoder::slotStartDecoding()
{
   //TODO set endpoint
   startDecode( "" ) ;
}
